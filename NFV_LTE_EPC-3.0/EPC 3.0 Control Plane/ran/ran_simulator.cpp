...
void simulate(int arg) {
	...
	while (1) {
		...
		if(ok == 0)
		{
			TRACE(cout << "Ransim_authenticate: did not receive packet,  "<< endl;)
			continue;
		}
		if (ok == -1) {
		...
		if(ok == 0)
		{
			TRACE(cout << "Ransim_set_security: did not receive packet,  "<< endl;)
			continue;
		}
		if (ok == -1) {
		...
		if(ok == 0)
		{
			TRACE(cout << "Ransim_set_eps_session: did not receive packet,  "<<  endl;)
			continue;
		}
		if (ok == -1) {
		...
		*/ 
		...
		if(ok == 0)
		{
			TRACE(cout << "Ransim_detach: did not receive packet,  "<< endl;)
			continue;
		}
		if (ok == -1) {
