...
//CHANGE THESE, then go to line 235 and 265
...
/*----------------------------------------------------------------------------*/
int check_ipaddr(struct iphdr* iph, char src[], char dest[])
{
...
#endif
//trishalDumpPacketO(mtcp, (char *)npc->snd_pktbuf[idx], pkt_size, "OUTTTT");			
 tx_again:
...
netmap_recv_pkts(struct mtcp_thread_context *ctxt, int ifidx)
{
	...
	for (c = 0; c < n && cnt != got && npc->dev_poll_flag[ifidx]; c++) {
		...
		for ( ; !nm_ring_empty(ring) && cnt != got; got++) {
			...
			//added for Data Plane
            
            unsigned char *capture_buffer = npc->rcv_pktbuf[p];
            int length_pkt = npc->rcv_pkt_len[p];
            struct ethhdr *ethh = (struct ethhdr *)capture_buffer;
            struct iphdr* iph = (struct iphdr *)(capture_buffer + sizeof(struct ethhdr));
            ...
            if(ntohs(ethh->h_proto) == ETH_P_IP && check_ipaddr(iph, DP_RAN_IP,DP_SINK_IP ))
    		{	//printf("\n IP SRC %s IP DEST %s\n",dest_buf,src_buf);
        		uint8_t *buffer_send = netmap_get_wptr(ctxt, CONFIG.eths[ifidx].ifindex, length_pkt);
				if (!buffer_send) {
					//TRACE_DBG("Failed to get available write buffer\n");
					printf("Error Buffer NULL\n");
					return -1;
				}
				memcpy(buffer_send, capture_buffer, length_pkt);
				struct ethhdr *ethhz = (struct ethhdr *)buffer_send;
				...
				/* Enter PGW MAC  */
				ethhz->h_source[0] = 0x00;
				ethhz->h_source[1] = 0xaa;
				ethhz->h_source[2] = 0xbb;
				ethhz->h_source[3] = 0xcc;
				ethhz->h_source[4] = 0xdd;
				ethhz->h_source[5] = 0x16;
    		}	
    		if(ntohs(ethh->h_proto) == ETH_P_IP && check_ipaddr(iph, DP_SINK_IP,DP_RAN_IP ))
    		{	//printf("\n IP SRC %s IP DEST %s\n",dest_buf,src_buf);
        		uint8_t *buffer_send = netmap_get_wptr(ctxt, CONFIG.eths[ifidx].ifindex, length_pkt);
				if (!buffer_send) {
					//TRACE_DBG("Failed to get available write buffer\n");
