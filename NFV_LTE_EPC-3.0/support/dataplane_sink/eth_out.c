...
//Change IP address and go to line 101
#define DP_RAN_MAC "00:aa:bb:cc:dd:02"
#define DP_SINK_MAC "00:aa:bb:cc:dd:17"
/*----------------------------------------------------------------------------*/

int check_mac(struct ethhdr *ethh, char src[], char dest[])
{
    char src_buf[20];
    char dest_buf[20];
    sprintf(src_buf, "%02x:%02x:%02x:%02x:%02x:%02x", ethh->h_source[0], ethh->h_source[1], ethh->h_source[2], ethh->h_source[3], ethh->h_source[4], ethh->h_source[5]);
    sprintf(dest_buf, "%02x:%02x:%02x:%02x:%02x:%02x", ethh->h_dest[0], ethh->h_dest[1], ethh->h_dest[2], ethh->h_dest[3], ethh->h_dest[4], ethh->h_dest[5]);
		...
		int nif, unsigned char* dst_haddr, uint16_t iplen)
{
	...
	if(check_mac(ethh, DP_SINK_MAC, DP_RAN_MAC))
    	{
       		    /* Enter PGW MAC  */
				ethh->h_dest[0] = 0x00;
				ethh->h_dest[1] = 0xaa;
