...
extern string g_mme_rep_ip1;
extern string g_mme_rep_ip2;
...
extern int g_mme_rep_port;
extern vector<UdpClient> mme_rep_clients1;
extern vector<UdpClient> mme_rep_clients2;


//const string dsmme_path = "10.129.28.112:8090";
const string dsmme_path = PDS;

//const string dsmme_path = "10.129.28.44:7001";
...
public:
	...
	void handle_create_session(int, Packet, vector<UdpClient>&,UdpClient& sgw_client,int);
	void handle_attach_complete(Packet,int);
	void handle_modify_bearer(int,Packet, vector<UdpClient>&,UdpClient& ,int);
	void handle_detach(int, Packet, vector<UdpClient>&,UdpClient& ,int);
	UeContext get_context(uint64_t guti);

	void push_context(uint64_t ,UeContext ,uint32_t ,int );
	bool pull_context(Packet ,int );
	void erase_context(uint32_t,int);

	void send_replication(int ,Mme_state ,uint32_t ,uint64_t ,int);
