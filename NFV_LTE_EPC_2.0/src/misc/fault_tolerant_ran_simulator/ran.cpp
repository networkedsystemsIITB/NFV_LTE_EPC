...
int g_mme_port = 5000;
string g_sgw_s1_ip_addr = "10.129.26.202";
...
bool Ran::initial_attach() {
	...
	err_code = mme_client_attach_1.snd(pkt);/*XCHANGE 1*/	
	TRACE(cout << "ran_initialattach:" << " request sent for ran: " << ran_ctx.imsi << endl;)
	

	err_code = mme_client_attach_1.rcv(pkt);/*XCHANGE 1*/
...
bool Ran::authenticate() {
	...
	//#warning "Change this"
	//TODO refactor this thing into a proper switch case
	if(UE_BINDING==STATE_LESS){
		err_code = mme_client_attach_2.snd(pkt);/*XCHANGE 2*/
	}	
	else if(UE_BINDING==SESSION_BASED){
		err_code = mme_client_attach_1.snd(pkt);/*XCHANGE 2*/			
	...
	if(DBG)cout<<"sent step2"<<"||"<< pkt.len<<endl;
	if(UE_BINDING==STATE_LESS){
		err_code  = mme_client_attach_2.rcv(pkt);/*XCHANGE 2*/
	}	
	else if(UE_BINDING==SESSION_BASED){
		err_code = mme_client_attach_1.rcv(pkt);/*XCHANGE 2*///cout<<"sess based xch2"<<endl;
	}
	else if(UE_BINDING==UE_BASED){
		err_code = mme_client_attach_1.rcv(pkt);/*XCHANGE 2*/
	}
	...
	if(DBG)	cout<<"received step2"<<"||"<< pkt.len<<endl;
...
bool Ran::set_security(TrafficMonitor &traf_mon) {
	...
	if(DBG)cout<<"step 3  sent"<<"||"<< pkt.len<<endl;
	...
	if(DBG)cout<<"step 3 received"<<"||"<< pkt.len<<endl;
...
bool Ran::set_eps_session(TrafficMonitor &traf_mon) {
	...
	ran_ctx.emm_state = 1;
	ran_ctx.ecm_state = 1;
	if(DBG)cout<<"step 4 sent"<<"||"<< pkt.len<<endl;
...
bool Ran::detach() {
	...
	if(DBG)cout<<__FILE__<<"||"<<__LINE__<<"||"<< pkt.len<<endl;
	...
	if(UE_BINDING==STATE_LESS){		
		...
		if(DBG)	cout<<"receive 5 sent"<<"||"<< pkt.len<<endl;


	if (pkt.len <= 0) {//cout<<"++++++++++++++++++++++++++++"<<err_code<<"++++++++++++++++++++++++++packet length zero"<<endl;
		cout<<"did not receive anything"<<endl;
		return false;
	}	
