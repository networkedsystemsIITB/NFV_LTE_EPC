...
int SctpClient::snd(Packet pkt) {
	...
	while (1) {
		...
		if (errno == EPERM) {
			...
			err_code = -1;//cout<<"===========================================ERRORCODE:"<<err_code<<endl;
			close(conn_fd);errno = 0;	sleep(5);
			conn_refresh();	
			break;	
		}
		else {
			
			//if(errno!=0)cout<<"error at snd occuredes"<<errno<<endl;
			break;
		}		
	}
...
int SctpClient::rcv(Packet &pkt) {
	...
	if(errno == EAGAIN || errno == EPIPE || errno == EBADF || errno == ECONNRESET){
		...
		//	if(errno!=0)cout<<"error at rcvoccuredes"<<errno<<endl;
			//break;
		}	
